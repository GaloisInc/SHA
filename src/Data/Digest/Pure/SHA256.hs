-- |This module implements the core SHA256 routines: sha224 and sha256.
-- You perform a hash by creating an initial SHA256State, advance it
...
module Data.Digest.Pure.SHA256(
         SHA256State
       , initialSHA224State
       , initialSHA256State
       , customSHA256State
...
-- |The running state of an ongoing SHA224/SHA256 hash.
data SHA256State  = SHA256S Word64 (Decoder SHA256Values)
data SHA256Values = SHA256V !Word32 !Word32 !Word32 !Word32
                            !Word32 !Word32 !Word32 !Word32

-- |The official initial state for running SHA224.
initialSHA224State :: SHA256State
initialSHA224State = SHA256S 0 (decoderSHA256 initVals)
 where initVals = SHA256V 0xc1059ed8 0x367cd507 0x3070dd17 0xf70e5939
                          0xffc00b31 0x68581511 0x64f98fa7 0xbefa4fa4

-- |The official initial state for running SHA256.
initialSHA256State :: SHA256State
initialSHA256State = SHA256S 0 (decoderSHA256 initVals)
 where initVals = SHA256V 0x6a09e667 0xbb67ae85 0x3c6ef372 0xa54ff53a
                          0x510e527f 0x9b05688c 0x1f83d9ab 0x5be0cd19

-- |Create an initial state for a running SHA224 or SHA256 hash from the given
-- lazy ByteString. The ByteString should be exactly 32 bytes long.
customSHA256State :: L.ByteString -> SHA256State
customSHA256State = importSHAState "SHA256" getSHA256State
 where
  getSHA256State =
    do [a, b, c, d, e, f, g, h] <- replicateM 8 getWord32be
       return (SHA256S 0 (Done S.empty 0 (SHA256V a b c d e f g h)))

-- |Create an initial state for a running SHA224 or SHA256 hash from the given
-- strict ByteString. The ByteString should be exactly 32 bytes long.
customSHA256State' :: S.ByteString -> SHA256State
customSHA256State' = customSHA256State . L.fromStrict

-- |Add the given lazy ByteString to the running hash.
advanceSHA256State :: SHA256State -> L.ByteString -> SHA256State
advanceSHA256State = advanceLazy advanceSHA256State'

-- |Add the given strict ByteString to the running hash.
advanceSHA256State' :: SHA256State -> S.ByteString -> SHA256State
advanceSHA256State' (SHA256S n decoder) c =
  advanceState SHA256S n c decoderSHA256 decoder

-- |Finalize a SHA224 state into a lazy ByteString.
finalizeSHA224State :: SHA256State -> L.ByteString
finalizeSHA224State state@(SHA256S n _) =
  let pad                     = generatePad 448 512 n 8
      getValue (SHA256S _ x)  = x
      SHA256V a b c d e f g _ = finalState getValue advanceSHA256State state pad
  in runPut $ putWord32be a >> putWord32be b >> putWord32be c >>
              putWord32be d >> putWord32be e >> putWord32be f >>
              putWord32be g

-- |Finalize a SHA224 state into a strict ByteString.
finalizeSHA224State' :: SHA256State -> S.ByteString
finalizeSHA224State' = L.toStrict . finalizeSHA224State

-- |Finalize a SHA256 state into a lazy ByteString.
finalizeSHA256State :: SHA256State -> L.ByteString
finalizeSHA256State state@(SHA256S n _) =
  let pad                     = generatePad 448 512 n 8
      getValue (SHA256S _ x)  = x
      SHA256V a b c d e f g h = finalState getValue advanceSHA256State state pad
  in runPut $ putWord32be a >> putWord32be b >> putWord32be c >>
              putWord32be d >> putWord32be e >> putWord32be f >>
              putWord32be g >> putWord32be h

-- |Finalize a SHA256 state into a strict ByteString.
finalizeSHA256State' :: SHA256State -> S.ByteString
finalizeSHA256State' = L.toStrict . finalizeSHA256State

-- -----------------------------------------------------------------------------
--
-- SHA256 Advancement routine
--
-- -----------------------------------------------------------------------------

decoderSHA256 :: SHA256Values -> Decoder SHA256Values
decoderSHA256 = runGetIncremental . advanceSHA256

advanceSHA256 :: SHA256Values -> Get SHA256Values
advanceSHA256 s00@(SHA256V a00 b00 c00 d00 e00 f00 g00 h00) =
  do w00 <- getWord32be
     w01 <- getWord32be
     w02 <- getWord32be
     w03 <- getWord32be
     ...
     let w16 = lsig256_1 w14 + w09 + lsig256_0 w01 + w00
         ...
         SHA256V a64 b64 c64 d64 e64 f64 g64 h64 = s64
     return $ SHA256V (a00 + a64) (b00 + b64) (c00 + c64) (d00 + d64)
                      (e00 + e64) (f00 + f64) (g00 + g64) (h00 + h64)

step256 :: SHA256Values -> Word32 -> Word32 -> SHA256Values
step256 (SHA256V a b c d e f g h) k w = SHA256V a' b' c' d' e' f' g' h'
 where
